...
export default function PublicationCard({ publication }: PublicationCardProps) {
  const { title, authors, venue, year, status, abstract, paperUrl, arxivUrl, doi, githubUrl } =
    publication

  const links: PublicationLink[] = [
    paperUrl ? { label: 'Paper', href: paperUrl, icon: FiFileText } : null,
    arxivUrl ? { label: 'arXiv', href: arxivUrl, icon: FiExternalLink } : null,
    doi ? { label: 'DOI', href: `https://doi.org/${doi}`, icon: FiHash } : null,
    githubUrl ? { label: 'Code', href: githubUrl, icon: FiGithub } : null,
    ...
    <article className="py-8 first:pt-0">
      ...
      <p className="mt-1.5 font-mono text-sm text-text-muted">
        {authors.map((author, index) => (
          <span key={author} className={author === PRIMARY_AUTHOR ? 'text-text-primary' : ''}>
            {author}
      ...
      <p className="mt-4 max-w-2xl text-sm leading-relaxed text-text-muted">{abstract}</p>
